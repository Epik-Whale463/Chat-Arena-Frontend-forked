...
export function useVotingGuide() {
  ...
  const hasSeenVotingGuide = useCallback(() => {
    return localStorage.getItem('voting_guide_seen_tts') === 'true';
  }, []);
  ...
  const checkAndShowVotingGuide = useCallback(() => {
    ...
    const hasSeenGuide = localStorage.getItem('voting_guide_seen_tts') === 'true';
  ...
  const markVotingGuideAsSeen = useCallback(() => {
    localStorage.setItem('voting_guide_seen_tts', 'true');
    localStorage.setItem('voting_guide_seen_tts_timestamp', new Date().toISOString());
  }, []);
