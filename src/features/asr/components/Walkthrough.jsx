...
export function Walkthrough() {
  ...
  const steps = [
    {
      target: 'body',
      title: 'Welcome to ASR Arena! 🎉',
      content: (
        <div className="text-center space-y-3">
          <p className="text-gray-700 text-base leading-relaxed">
            Experience and compare the best <strong>Speech-to-Text</strong> models in <strong>Indian languages</strong>.
          </p>
  ...
  useEffect(() => {
    const hasSeenWalkthrough = localStorage.getItem('has_seen_walkthrough_asr') === 'true';
  ...
  useEffect(() => {
    if (currentStep >= 0 && currentStep < steps.length && isActive) {
      ...
      const needsSidebar = ['[data-tour="asr-sidebar"]', '[data-tour="asr-new-chat"]', '[data-tour="asr-leaderboard-link"]'].includes(step.target);
    ...
    if (prevAutoOpened) {
      const isMobile = typeof window !== 'undefined' && window.innerWidth < 768;
      const needsSidebar = currentStep >= 0 && currentStep < steps.length && ['[data-tour="asr-sidebar"]', '[data-tour="asr-new-chat"]', '[data-tour="asr-leaderboard-link"]'].includes(steps[currentStep].target);
      if (isMobile && !needsSidebar) {
        closeSidebarMobile();
  ...
  const openSidebarMobile = () => {
    const sidebar = document.querySelector('[data-tour="asr-sidebar"]');
  ...
  const updateTooltipPosition = (selector, placement, avoidCovering = false) => {
    ...
    let top = 0;
    let left = 0;
    const forceRightPlacement = selector === '[data-tour="asr-mode-selector"]' && window.innerWidth >= 768;
    ...
    switch (currentPlacement) {
      case 'bottom':
        ...
        if (selector === '[data-tour="asr-mode-selector"]') {
    ...
    const isModeDesktop = selector === '[data-tour="asr-mode-selector"]' && window.innerWidth >= 768;
  ...
  const handleFinish = () => {
    localStorage.setItem('has_seen_walkthrough_asr', 'true');
    closeSidebarMobile();
    autoOpenedSidebarRef.current = false;
    setIsActive(false);
    setCurrentStep(-1);
  ...
  const forceCenteredArrow =
    ...
    isDesktop &&
    orientation === 'bottom' &&
    (step.target === '[data-tour="asr-mode-selector"]' || step.target === '[data-tour="asr-leaderboard-link"]');
      ...
      <div
        className={`fixed z-[10000] bg-white rounded-xl border-2 border-orange-500 transition-all duration-200 ${
          isCentered
            ...
            : step.target === '[data-tour="asr-mode-selector"]' && window.innerWidth >= 768
